...
<div>Candidate module: Intern Systems Qualification • Revision 7.3 • CAT-ITICAL risk classification</div>
</div>
""", unsafe_allow_html=True)
...
with left:
    if st.session_state.finished:
        ...
        key = st.text_input("Keyboard command", placeholder="Type W, A, S, or D and press Enter", max_chars=1, label_visibility="collapsed").lower()

        drdc = None
        if up or key == "w": drdc = (-1, 0)
        elif down or key == "s": drdc = (1, 0)
        elif left_btn or key == "a": drdc = (0, -1)
        elif right_btn or key == "d": drdc = (0, 1)
        ...
        if drdc:
            ...
            if maze[nr][nc] == 0:
                ...
                st.session_state.message = random.choice([
                    ...
                    "Access denied. Submit form TAF-catfurr and try another route.",
                    "Wall contact recorded. Quality has been copied on the email.",
                    "Invalid move. The cat auditor appears disappointed.",
                ])
            st.rerun()

st.caption("For entertainment only. No interns, cats, capybaras, or quality engineers were harmed during validation.")
